 ...
 * Resolve
 */
const resolveFilePath = coroutine(function * () {
 ...
 */
const chunk = coroutine(function * (filePath, parent) {
  if (graph.getNode(filePath)) {
    return null
  }

  var normalizedPath = yield resolveFilePath
})

const testCoroutine = coroutine(function * () {
  throw new Error('test')
})

module.exports = coroutine(function * (filePath, allowExternal) {
  //var data = yield file.readFile(filePath)
  var data = yield testCoroutine()
