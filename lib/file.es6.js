...
const resolve = require('./graph').resolve;
...
const logErr = function(args) {
  ...
  console.log(chalk.cyan('[br]'), chalk.read('[error]'), args.join(','));
};
...
file.buildDepsGraph = function(file) {
  ...
  (file, parent) => {
    ...
    const data = (urlRegex({ exact: true }).test(file)) ? self.load(file) : self.readFile(file);
    ...
    const required = self.parseHeader(data);
...
file.load = function(url) {
  ...
  try {
    const res = request('GET', url).getBody('utf-8');
  } catch (error) {
    logErr(error);
  }
  return res;
...
file.watch = function(args, cb) {
  const watcher = chokidar.watch(args, { ignoreInitial: true });
  watcher.on('ready', () => {
    console.log(chalk.white(
      chalk.cyan('[br]'),
